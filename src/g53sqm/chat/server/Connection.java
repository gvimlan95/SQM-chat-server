...
public class Connection implements Runnable {
    ...
    private void stat() {
        String status = "STAT There are currently "+serverReference.getNumberOfUsers()+" user(s) on the server ";
        switch(state) {
    ...
    private void list() {
        switch(state) {
            case STATE_REGISTERED:
                ...
                sendOverConnection("USERLIST " + userListString);
                break;
        ...
        switch(state) {
            ...
            case STATE_UNREGISTERED:
                ...
                if(serverReference.doesUserExist(username)) {
                    ...
                    sendOverConnection("REGDONE Welcome to the chat server " + username);
                }
                break;
        }
    }
    ...
    private void hail(String message) {
        switch(state) {
            case STATE_REGISTERED:
                serverReference.broadcastMessage("BROADCAST Broadcast from " + username + ": " + message);
                messageCount++;
                break;
    ...
    private void quit() {
        switch(state) {
            case STATE_REGISTERED:
                sendOverConnection("QUIT thank you for sending " + messageCount + " message(s) with the chat service, goodbye. ");
                break;
            case STATE_UNREGISTERED:
                sendOverConnection("QUIT goodbye");
                break;
        }
        running = false;
        try {
